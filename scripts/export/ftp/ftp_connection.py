...
from ...gui.common.gui_classes import Function_Worker
...
class FTP_Connection:
    ...
    def try_to_execute(self, method: Callable[..., T], *args: Any) -> T | None:
        try:
            return method(*args)
    ...
    def check_connection(self) -> str | None:
        if self.connection is None:
            return None
        return self.try_to_execute(self.connection.voidcmd, "NOOP")

    def nlst(self) -> list[str] | None:
        if self.connection is not None:
            return self.try_to_execute(self.connection.nlst)
        return None

    def mkd(self, dirname: str) -> str | None:
        if self.connection is not None:
            return self.try_to_execute(self.connection.mkd, dirname)
        return None

    def storbinary(self, cmd: str, fp: str) -> str | None:
        if self.connection is not None:
            return self.try_to_execute(self.connection.storbinary, cmd, fp)
        return None

    def close(self) -> str | None:
        if self.connection is not None:
            return self.try_to_execute(self.connection.close)
        return None
