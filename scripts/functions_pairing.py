...
def get_pairings_fold(n, first_seed_white=True):
    pairings = [(i, n - i - 1 - (n % 2)) for i in range(int(n / 2))]
    for i in range(first_seed_white, len(pairings), 2):
        pairings[i] = (pairings[i][1], pairings[i][0])
    if n % 2:
        pairings.append((n - 1, n))
...
def get_pairings_random(n, first_seed_white=True):
...
def get_pairings_cycle(n, i):
    bye = n % 2
    n += n % 2
    div, mod = divmod(i - 1, n - 1)
    pairings = [(n - 1, mod)]
    pairings.extend([((mod - j) % (n - 1), (mod + j) % (n - 1)) for j in range(1, int(n / 2))])
...
def get_pairings_berger(n, i):
    ...
    if mod % 2:
        ...
        pivot = int(mod / 2)
    pairings = [(pivot, n - 1)]
    pairings.extend([((pivot + sign * j) % (n - 1), (pivot - sign * j) % (n - 1)) for j in range(1, int(n / 2))])
    if mod % 2 != div % 2:
